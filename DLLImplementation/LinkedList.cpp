...
void LinkedList::delRec(std::string name, int select) {
	switch (select) {
	case 1: {
		LinkedList::Result* result = this->searchByFirst(name);
		if (result == nullptr) { std::cout << "Illegal Operation: First Name not found on Linked List.\n" << std::endl; }
		else {
			if (result->count == 1) {
				Node* temp = result->arr[0];
				if (temp->getPrev() == nullptr) {
					if (temp->getNext() == nullptr) {
						this->start = nullptr;
						delete temp;
		  ...
		  break;
	case 2: {
		LinkedList::Result* result = this->searchByLast(name);
		if (result == nullptr) { std::cout << "Illegal Operation: Last Name not found on Linked List.\n" << std::endl; }
		else {
			if (result->count == 1) {
				Node* temp = result->arr[0];
				if (temp->getPrev() == nullptr) {
	...
	default: {
		LinkedList::Result* result = this->searchByFirst(name);
		if (result == nullptr) { std::cout << "Illegal Operation: First Name not found on database.\n" << std::endl; }
		else {
			for (int i = 0; i < result->count; i++) {
				Node* temp = result->arr[i];
				temp->getPrev()->setNext(temp->getNext());
			...
			LinkedList::Result* result = this->searchByLast(name);
			if (result == nullptr) { std::cout << "Illegal Operation: Last Name not found on database.\n" << std::endl; }
			else {
				for (int i = 0; i < result->count; i++) {
					Node* temp = result->arr[i];
					temp->getPrev()->setNext(temp->getNext());
